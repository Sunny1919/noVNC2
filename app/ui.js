...
* ------v------*/
    ...
    addRecordingHandlers() {
        // Screenshot button
        const screenshotBtn = document.getElementById("noVNC_screenshot_button");
        if (screenshotBtn) {
            screenshotBtn.addEventListener('click', UI.takeScreenshot);
        }
 ...
 * ------v------*/
    ...
    clipboardReceive(e) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(clipboardText)
                .then(() => {
                    Log.Info("Clipboard synced to browser successfully");
                    UI.showStatus(_("Clipboard synced from server"), 'normal', 2000);
                })
                .catch(err => {
                    Log.Warn("Failed to sync clipboard to browser: " + err);
                    // Fallback: Thử dùng execCommand nếu Clipboard API không hoạt động
                    try {
                        ...
                        Log.Info("Clipboard synced using fallback method");
                    } catch (fallbackErr) {
                        Log.Error("All clipboard sync methods failed: " + fallbackErr);
                    }
                });
        } else {
            Log.Warn("Clipboard API not available, clipboard sync disabled");
        }
    ...
    // Tự động đồng bộ clipboard từ browser khi người dùng copy
    setupAutomaticClipboardSync() {
        if (!navigator.clipboard || !navigator.clipboard.readText) {
            Log.Warn("Clipboard read API not available");
            return;
        }
        
        // Lắng nghe sự kiện focus vào window để kiểm tra clipboard
        ...
        const checkClipboard = async () => {
            ...
            try {
                ...
                if (text && text !== lastClipboardText) {
                    ...
                    Log.Info("Browser clipboard changed, syncing to server");
                    UI.rfb.clipboardPasteFrom(text);
                    document.getElementById('noVNC_clipboard_text').value = text;
                }
            } catch (err) {
                // Người dùng có thể từ chối quyền truy cập clipboard
                Log.Debug("Could not read clipboard: " + err);
            }
        };
        
        // Kiểm tra clipboard khi window được focus
        window.addEventListener('focus', checkClipboard);
        
        // Kiểm tra clipboard định kỳ khi đang kết nối (mỗi 1 giây)
        UI.clipboardSyncInterval = setInterval(() => {
            if (UI.connected && document.hasFocus()) {
                checkClipboard();
            }
        }, 1000);
    },
    
    stopAutomaticClipboardSync() {
        if (UI.clipboardSyncInterval) {
            clearInterval(UI.clipboardSyncInterval);
            UI.clipboardSyncInterval = null;
        }
    },
 ...
 * ------v------*/
    ...
    async takeScreenshot() {
        ...
        try {
            ...
            if (!UI.screenshotUtil) {
                const { CanvasScreenshot } = await import('../core/recorder.js');
                const canvas = document.getElementById('noVNC_canvas');
                UI.screenshotUtil = new CanvasScreenshot(canvas);
            }

            // Capture and download
            await UI.screenshotUtil.captureAndDownload('image/png', 1.0);
            UI.showStatus(_("Screenshot saved"), 'normal', 2000);

        } catch (err) {
            Log.Error('Screenshot failed:', err);
            UI.showStatus(_("Screenshot failed"), 'error');
        }
    },
    ...
    async toggleRecording() {
        ...
        try {
            // Lazy load recorder module
            if (!UI.recorder) {
                const { CanvasRecorder } = await import('../core/recorder.js');
                const canvas = document.getElementById('noVNC_canvas');
                UI.recorder = new CanvasRecorder(canvas);
            }
            ...
            if (UI.recorder.isRecording) {
                ...
                // Start recording
                const success = await UI.recorder.startRecording({
                    frameRate: 30,
                ...
                if (success) {
                    ...
                    UI.recordingInterval = setInterval(() => {
                        if (UI.recorder && UI.recorder.isRecording) {
                            const duration = Math.floor(UI.recorder.getDuration() / 1000);
                            const minutes = Math.floor(duration / 60);
                            const seconds = duration % 60;
                            const timeStr = `${minutes}:${seconds.toString().padStart(2, '0')}`;
                            
                            const btn = document.getElementById('noVNC_record_button');
                            if (btn) {
                                btn.title = `Recording: ${timeStr}`;
                            }
                        }
                    }, 1000);
                } else {
            ...
            UI.showStatus(_("Recording failed"), 'error');
        }
    },
    ...
    updateRecordButton(isRecording) {
        ...
        if (btn) {
            if (isRecording) {
                btn.classList.add('noVNC_recording');
                btn.title = 'Stop Recording';
            } else {
                btn.classList.remove('noVNC_recording');
                btn.title = 'Start Recording';
                
                if (UI.recordingInterval) {
                    clearInterval(UI.recordingInterval);
                    UI.recordingInterval = null;
                }
            }
        }
    },
 ...
 * ------v------*/
    ...
    async initBandwidthAdapter() {
        try {
            ...
            Log.Info('Bandwidth adapter initialized');
        } catch (err) {
            Log.Error('Failed to initialize bandwidth adapter:', err);
        }
    },

    // Enable/disable bandwidth adaptation
    toggleBandwidthAdaptation(enable) {
 ...
 * ------v------*/
    ...
    async initAudioStream() {
        try {
            const { AudioStreamPlayer, PulseAudioClient } = 
                await import('../core/audio-stream.js');
            ...
            audioBtn.id = 'noVNC_audio_toggle_button';
    ...
    async toggleAudio() {
        if (!UI.audioPlayer) {
            UI.showStatus(_("Audio not available"), 'error');
            return;
        }

        const btn = document.getElementById('noVNC_audio_toggle_button');
        
        if (UI.audioPlayer.isPlaying) {
            UI.audioPlayer.stop();
            if (btn) btn.textContent = '🔇';
            UI.showStatus(_("Audio stopped"), 'normal', 2000);
        } else {
            await UI.audioPlayer.start();
            if (btn) btn.textContent = '🔊';
            UI.showStatus(_("Audio started"), 'normal', 2000);
        }
    },
 ...
 * ------v------*/
    ...
    async initFrameScheduler() {
        try {
            const { patchDisplay } = await import('../core/display-optimizer.js');
            
            // Frame scheduler will be patched when RFB connects
            UI.frameSchedulerReady = true;
            
            Log.Info('Frame scheduler ready');
        } catch (err) {
            Log.Error('Failed to initialize frame scheduler:', err);
        }
    },

    // Patch display with frame scheduler
    patchDisplayWithScheduler(display) {
        if (!UI.frameSchedulerReady) {
            return display;
        }

        try {
            const { patchDisplay } = require('../core/display-optimizer.js');
            const patchedDisplay = patchDisplay(display);
            ...
            return patchedDisplay;
        } catch (err) {
            Log.Error('Failed to patch display:', err);
            return display;
        }
    },
    ...
    showFpsCounter() {
        ...
        fpsCounter.id = 'noVNC_fps_counter';
        ...
        // Update FPS counter
        setInterval(() => {
            if (!UI.rfb || !UI.rfb._display || !UI.rfb._display.getFrameStats) {
                return;
            }

            const stats = UI.rfb._display.getFrameStats();
            fpsCounter.textContent = `${stats.fps} FPS`;
            fpsCounter.style.display = 'block';
            
            // Color based on FPS
            if (stats.fps >= 55) {
                fpsCounter.style.color = '#0f0'; // Green
            } else if (stats.fps >= 30) {
                fpsCounter.style.color = '#ff0'; // Yellow
            } else {
                fpsCounter.style.color = '#f00'; // Red
            }
        }, 1000);
    },
    ...
    // Toggle FPS counter
    toggleFpsCounter() {
        const counter = document.getElementById('noVNC_fps_counter');
        if (counter) {
 ...
 * ------v------*/
    ...
    disconnectFinished(e) {
        ...
        // Dừng tự động đồng bộ clipboard
        UI.stopAutomaticClipboardSync();
